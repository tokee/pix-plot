...
// Large textures base setup
// Requires atlas files generated by process_images.py for the requested size
var largeTextureSide = 128;
var largeTextureScale = largeTextureSide/32;
...
function loadLargeAtlasFiles() {
  sizes.image = {
    width: largeTextureSide,
    height: largeTextureSide
  }
  sizes.atlas = {
    width: 2048,
    height: 2048,
    cols: 2048 / largeTextureSide,
    rows: 2048 / largeTextureSide
  }
  sizes.scaleFactor = 4;
  for (var i=0; i<atlasCounts['128px']; i++) {
    var url = dataUrl + 'atlas_files/' + largeTextureSide + 'px/atlas-' + i + '.jpg';
...
function handleLargeTexture(atlasIndex, texture) {
  var material = new THREE.MeshBasicMaterial({ map: texture });
  materials['large'][atlasIndex] = material;
...
function updateImages(atlasIndex) {
  ...
  meshes[meshIndex].material.push( materials['large'][atlasIndex] )
...
function getAtlasImages(atlasIdx, materialIdx) {
  ...
  imageDataKeys.slice(startImage, endImage).forEach(function(d) {
    ...
    img.width *= sizes.scaleFactor;
    img.height *= sizes.scaleFactor;
    img.xOffset *= sizes.scaleFactor;
    img.yOffset *= sizes.scaleFactor;
    // Get atlas information for this image
